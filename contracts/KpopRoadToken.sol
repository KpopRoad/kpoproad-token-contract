...
contract K_POP_ROAD_Coin is ERC20,ERC20Burnable, Ownable {
    ...
    function antiBot(address _account , bool _open) external onlyOwner {
        require(_account != address(0), "Cannot blacklist zero address");
        require(_account != owner(), "Cannot blacklist owner");
        _blacklist[_account] = _open;
    }



    modifier validTransfer(address _from, address _to) {
        require(!_blacklist[_from], "Sender is blacklisted");
        require(!_blacklist[_to], "Recipient is blacklisted");
        _;
    }


    function transfer(address to, uint256 value) public override validTransfer(msg.sender, to) returns (bool) {
        return super.transfer(to, value);
    }
    
    function transferFrom(address from, address to, uint256 value) public override validTransfer(from, to) returns (bool) {
        return super.transferFrom(from, to, value);
    }


    // ========== EMERGENCY FUNCTIONS ========== 
    function emergencyWithdrawMultipleTokens(address tokens, address to) external onlyOwner{
        if (tokens == address(0)) revert ZeroAddress();
        if (to == address(0)) revert ZeroAddress();
        uint256 balance = IERC20(tokens).balanceOf(address(this));
        IERC20(tokens).safeTransfer(to, balance);
    }
